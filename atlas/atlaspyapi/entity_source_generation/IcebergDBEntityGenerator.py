...
class IcebergDBEntityGenerator:
    __config = init_config(CONFIG_PATH)

    # get s3_bucket attributes list
    @staticmethod
    def get_iceberg_db_all_supported_attributes():
        return {
            "entity_type": "iceberg_db",
            "name": "Required attribute. "
                    "The name of the iceberg db, Example, insee-data",
            "clusterName": "Required attribute. "
                           " The cluster name of your iceberg db. Example, insee.org",
            "qualified_name": "Required attribute. "
                              " Fully qualified name of the iceberg db. It must be unique"
    ...
    @staticmethod
    def generate_iceberg_db_json_source(
        ...
        template_file_path = f"{TEMPLATE_FOLDER_PATH}/{IcebergDBEntityGenerator.__config.get(entity_type, 'template_name')}"
        # generate default value for optional empty attributes
        creator_id = kwargs.get(
            "creator_id", IcebergDBEntityGenerator.__config.get(entity_type, "createdBy")
        )
        updator_id = kwargs.get(
            "updator_id", IcebergDBEntityGenerator.__config.get(entity_type, "updatedBy")
        )
        create_time = kwargs.get("create_time", current_milli_time())
        update_time = kwargs.get("update_time", current_milli_time())
        owner = kwargs.get(
            "owner", IcebergDBEntityGenerator.__config.get(entity_type, "owner")
        )
